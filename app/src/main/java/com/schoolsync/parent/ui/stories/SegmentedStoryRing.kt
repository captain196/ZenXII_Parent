 ...
 *
 * Mirrors `com.schoolsync.teacher.ui.stories.SegmentedStoryRing` — both apps
 * render the same rings for the same stories, so any change here belongs in
 * the Teacher copy too (and vice versa).
 *
 * One arc per story, so a teacher with three stories shows three arcs around
